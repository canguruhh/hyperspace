...
fn test_reward_single_account() {
  ...
  ExtBuilder::default().build().execute_with(|| {
    let initial_balance = Currencies::total_balance(CurrencyId::ETP, &alice);
    run_to_block(10);
    ...
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &pool_account), 15 * DOLLARS);

    run_to_block(50);
    assert!(RPM::remove_share(&alice, pool_id, 100).is_ok(), "should remove shares to the pool");
    check_pool_data(&pool_id, &alice, 0, 0, 0, 0, 0);
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &alice), initial_balance + 40 * DOLLARS);
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &pool_account), 0);
  });
}
...
fn test_reward_single_account_existential() {
  ...
  ExtBuilder::default().build().execute_with(|| {
    ...
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &alice), initial_balance);
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &pool_account), 1 * DOLLARS);
    ...
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &alice), initial_balance + 10 * DOLLARS);
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &pool_account), 0);
  });
}
...
fn test_multi_account_rewards() {
  ...
  ExtBuilder::default().build().execute_with(|| {
    let initial_alice = Currencies::total_balance(CurrencyId::ETP, &alice);
    let initial_bob = Currencies::total_balance(CurrencyId::ETP, &bob);
    let initial_dave = Currencies::total_balance(CurrencyId::ETP, &dave);
    run_to_block(100);
    ...
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &pool_account), 0);
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &alice), initial_alice + 150 * DOLLARS);
    // rounding issue
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &bob), initial_bob + 175 * DOLLARS + 1);
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &dave), initial_dave + 75 * DOLLARS - 1);
  });
}
...
fn test_claim_rewards() {
 let pool_id = PoolId::Swap(1);
  ...
  ExtBuilder::default().build().execute_with(|| {
    ...
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &alice), alice_balance);
    ...
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &bob), bob_balance);
    assert_eq!(Currencies::total_balance(CurrencyId::ETP, &pool_account), 625 * DOLLARS / 10);
  });
}
